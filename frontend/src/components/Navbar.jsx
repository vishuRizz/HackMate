...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 z-50 flex items-center h-[55px] justify-between w-full px-6 py-3 bg-[#292929] text-white backdrop-blur-md shadow-md">
      ...
      <div className="hidden md:flex ml-4 flex-grow">
        <SearchBar width="w-64" height="h-[30px]" />
      </div>
      </div>
      ...
      <div className="md:hidden flex mr-2 items-center space-x-2 text-gray-300 cursor-pointer hover:text-yellow-400">
        <FiSearch size={23} onClick={() => setIsSearchOpen(!isSearchOpen)} />
      </div>
      ...
      <div className="hidden space-x-6 md:flex">
        {[
          ...
          >
            <item.icon size={25} className="text-2xl text-gray-300" />
      ...
      {/* Mobile Search Bar */}
      {isSearchOpen && (
        <div className="absolute top-[55px] right-0 w-full bg-[#292929] p-4 z-50">
          <SearchBar width="w-full" height="h-[35px]" />
        </div>
      )}
    </nav>
  );
